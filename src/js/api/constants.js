...
export const API_LISTING_DETAILS = (
  ...
  return `${API_LISTINGS}/${listingId}?${params}`;
};

// Search listings by title or description
export const API_LISTINGS_SEARCH = (query) =>
  `${API_LISTINGS}/search?q=${encodeURIComponent(query)}`;

// Create a new listing (POST request)
export const API_CREATE_LISTING = API_LISTINGS;

// Update an existing listing (PUT request)
export const API_UPDATE_LISTING = (listingId) => `${API_LISTINGS}/${listingId}`;

// Delete a listing (DELETE request)
export const API_DELETE_LISTING = (listingId) => `${API_LISTINGS}/${listingId}`;
 ...
 * @returns {string} - The constructed API endpoint URL
 */
export const API_PROFILE_LISTINGS = (profileName, page = 1, limit = 10) =>
  `${API_PROFILES}/${profileName}/listings?${new URLSearchParams({ _page: page, _limit: limit })}`;
...
export const API_PROFILE_BIDS = (profileName, page = 1, limit = 10) =>
  `${API_PROFILES}/${profileName}/bids?${new URLSearchParams({ _page: page, _limit: limit })}`;
