...
function renderSingleListing(listing) {
  ...
  listingContainer.innerHTML = `
    <div class="max-w-4xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden mt-20 p-6">
      <div class="relative">
        ${listing.media?.[0]?.url ? `<img src="${listing.media[0].url}" alt="${listing.media[0].alt || "Listing image"}" class="w-full h-96 object-cover rounded-lg" />` : `<div class="w-full h-96 bg-gray-200 flex items-center justify-center text-gray-500 rounded-lg">No Image Available</div>`}
      </div>
      <div class="mt-6">
        ...
        <div class="flex flex-wrap gap-2 mb-4">
          ${listing.tags?.length ? listing.tags.map((tag) => `<span class="bg-blue-100 text-blue-800 text-sm font-medium px-3 py-1 rounded-md">#${tag}</span>`).join(" ") : `<span class="text-gray-500">No tags available.</span>`}
        </div>
        ...
        <div id="bidSection" class="mt-6 p-4 bg-teal-800 rounded-lg shadow-md">
          ...
          <form id="bidForm" class="flex flex-col items-center gap-4">
            <input type="number" id="bidAmount" class="w-full p-2 rounded-md text-black" placeholder="Enter bid amount" min="1" required />
            <button type="submit" class="bg-amber-500 text-white px-4 py-2 rounded-md hover:bg-amber-600 transition">Place Bid</button>
          </form>
          <p id="bidMessage" class="text-center mt-2 text-sm"></p>
        </div>
        <div class="mt-6 flex gap-4">
          <a href="edit/?id=${listing.id}" class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600">Edit Listing</a>
